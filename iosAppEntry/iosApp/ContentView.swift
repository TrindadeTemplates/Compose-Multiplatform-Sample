...
struct ContentView: View {
  ...
  var body: some View {
    ComposeView(delegate: delegate).ignoresSafeArea()
  }
}
...
class RootHolder: ObservableObject {
  ...
  func restoreStateFromStateKeeper(coder: NSCoder) {
    appStateKeeper = StateKeeperDispatcherKt.StateKeeperDispatcher(savedState: StateKeeperUtilsKt.restore(coder: coder))
  }
}